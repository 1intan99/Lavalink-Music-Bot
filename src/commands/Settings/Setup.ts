...
import { generateSetup } from "../../utils/lavalink-function";
...
export default class SetupCommand extends Command {
    ...
    async exec(message: Message, args: string[]) {
        ...
        if (!data) {
            ...
            const msg = channel.messages.cache.find(m => m.id === data.musicId);

            if (!msg) {
                const gembed = generateSetup(message, this.client);

                const msg = await channel.send({ embeds: [...gembed.embeds], components: [...gembed.components] });
                ...
                if (msg.deletable) {
                    msg.delete().catch(err => Logger.log("ERROR", err));
                }
